...
TEST_POINT="codec_fidelity"; STATUS_SCHEMA="codec_fidelity_raw_status.v1"; INPUT_SCHEMA="codec_fidelity_inputs.v1"
...
class CodecFidelityExporter(ArtifactExporter):
    ...
    def export(self, context: ExportContext)->ArtifactBundle:
        refs={}; availability={}
        for split in SPLITS:
            p=context.input_root/f"codec_fidelity__raw_status__{split}.v1.json"
        ...
        path=context.store.write_json(TEST_POINT,"inputs",{"schema_version":INPUT_SCHEMA,"splits":refs,"availability":availability})
...
class CodecFidelityEvaluator(ArtifactEvaluator):
    ...
    def evaluate(self, context: EvaluationContext,bundle:ArtifactBundle)->EvaluationResult:
        ...
        if inputs.get("schema_version")!=INPUT_SCHEMA: raise ValueError("Unsupported codec fidelity inputs.")
        ...
        for split,ref in inputs.get("splits",{}).items():
            ...
            profiles[split]=_measure(resolver,status)
        if not profiles:
...
def _status(x,split):
    if x.get("schema_version")!=STATUS_SCHEMA or x.get("dataset",{}).get("split")!=split:raise ValueError("Codec status schema or split mismatch.")
...
def _markdown(r):
    ...
    lines=["# Codec 保真度","","| Split | 小节 | 可听声部 Chroma cosine | 调性条件 Chroma cosine | 音区中位数差（半音） |","| --- | ---: | ---: | ---: | ---: |"]
