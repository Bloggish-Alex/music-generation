...
class FormCandidate:
    """Represent one ranked form hypothesis and its supporting evidence."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize the candidate for reviewable metadata output."""
        return asdict(self)


@dataclass(frozen=True)
class SSMFeatures:
    """Store summary features derived from a square self-similarity matrix."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize similarity features as JSON-safe scalar values."""
        return asdict(self)
...
class MusicFileDiscoverer:
    ...
    def discover(self, music_dir: str | Path) -> List[Path]:
        """Return supported symbolic-music files recursively in stable order."""
        root = Path(music_dir)
        files: List[Path] = []
...
class FilenameFormClassifier:
    ...
    def classify(self, file_path: Path) -> Optional[FormCandidate]:
        """Return a high-confidence candidate when the filename names a form."""
...
class SSMBuilder:
    ...
    def __init__(self, distance: EditDistanceCalculator) -> None:
        """Bind the configured bar-distance calculator used by the SSM."""
        self.distance = distance

    def build_similarity(self, bars: Sequence[Any]) -> np.ndarray:
        """Return a square similarity matrix in `[0, 1]` for ordered bars."""
        if not bars:
            return np.zeros((0, 0), dtype=np.float64)
        self.distance.fit_corpus(bars)
        distance_matrix = self.distance.build_matrix(bars)
        # The configured edit distance is normalized, so complementary distance
        # gives a directly reviewable similarity value without another scale.
...
class SegmentSimilarity:
    ...
    def score(self, ssm: np.ndarray, first: tuple[int, int], second: tuple[int, int]) -> float:
        """Return mean pairwise similarity between two temporal regions."""
        a_start, a_end = first
    ...
    def template_ranges(self, template: FormTemplate, bar_count: int) -> List[tuple[str, int, int]]:
        """Scale relative template lengths to contiguous observed bar ranges."""
...
class SSMFormClassifier:
    ...
    def __init__(self, templates: FormTemplateLibrary) -> None:
        """Store available templates and the shared segment comparator."""
        self.templates = templates
        self.segment = SegmentSimilarity()

    def classify(self, ssm: np.ndarray) -> List[FormCandidate]:
        """Rank configured forms from global and form-specific SSM evidence."""
    ...
    def _features(self, ssm: np.ndarray) -> SSMFeatures:
        """Compute reusable recurrence and contrast evidence from an SSM."""
        n = int(ssm.shape[0])
        if n < 4:
    ...
    def _off_diagonal_mean(self, ssm: np.ndarray) -> float:
        """Measure average similarity excluding trivial self-matches."""
        if ssm.shape[0] <= 1:
            return 0.0
    ...
    def _within_half_consistency(self, ssm: np.ndarray) -> float:
        """Measure internal recurrence independently in both temporal halves."""
        n = ssm.shape[0]
        first = ssm[: n // 2, : n // 2]
        second = ssm[n // 2 :, n // 2 :]
    ...
    def _rondo_score(self, ssm: np.ndarray) -> float:
        """Measure recurrence among alternating A-like regions."""
        n = ssm.shape[0]
    ...
    def _variation_score(self, ssm: np.ndarray) -> float:
        """Measure recurrence from the opening segment to later partitions."""
        n = ssm.shape[0]
        segment_count = 4
        ranges = []
    ...
    def _sonata_score(self, ssm: np.ndarray) -> float:
        """Measure exposition-to-recap similarity across the outer thirds."""
    ...
    def _confidence(self, score: float, all_scores: Dict[str, float]) -> float:
        """Convert score level and margin into a bounded confidence value."""
...
class FormJsonRecordBuilder:
    ...
    def __init__(self, templates: FormTemplateLibrary) -> None:
        """Store templates used to expand a selected form into sections."""
        self.templates = templates
    ...
    def build(
        ...
        """Build one metadata record from ranked candidates and bar count."""
    ...
    def _template_sections(self, form_name: str, bar_count: Optional[int] = None) -> List[Dict[str, Any]]:
        """Scale template sections while preserving order and full coverage."""
...
class FormJsonGenerator:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        """Assemble filename, parser, SSM, classifier, and record stages."""
        self.config = config
    ...
    def generate(self, music_dir: str | Path) -> Dict[str, Any]:
        """Generate one reviewable metadata record per discovered music file."""
        result: Dict[str, Any] = {}
...
class FormJsonCLI:
    ...
    def build_parser(self) -> argparse.ArgumentParser:
        """Define the standalone form metadata command-line interface."""
    ...
    def run(self, argv: Optional[Sequence[str]] = None) -> None:
        """Validate overwrite policy, run generation, and write JSON artifacts."""
