...
class RootPitchClassFeatureConfig:
    """Configure the optional circular pitch-class distance penalty."""
...
class PositionBucketFeatureConfig:
    """Configure the optional relative song-position distance penalty."""
...
class EditDistanceConfig:
    """Select token channels, normalization, and optional context penalties."""
...
class EditDistanceDiagnosticsAnalyzer:
    ...
    def __init__(self, config: EditDistanceConfig) -> None:
        """Store thresholds and feature weights for diagnostic summaries."""
        self.config = config
    ...
    def summarize(
        ...
        """Summarize the upper triangle of a square pairwise distance matrix."""
    ...
    def _quantiles(self, values: np.ndarray) -> Dict[str, float]:
        """Return fixed distribution percentiles for pairwise distances."""
        if not values.size:
            return {}
        return {
    ...
    def _threshold_counts(
        ...
        """Count distance pairs at or below each diagnostic threshold."""
    ...
    def _value(self, value: Any) -> float:
        """Round NumPy or Python numeric values for stable JSON output."""
...
class EditDistanceCalculator:
    ...
    def __init__(self, config: EditDistanceConfig) -> None:
        """Create a calculator and its optional learned bar feature extractor."""
        self.config = config
    ...
    def from_style_config(cls, config: Dict[str, Any]) -> "EditDistanceCalculator":
        """Build a calculator from distance and autoencoder configuration sections."""
    ...
    def fit_corpus(self, bars: Sequence[BarRecord]) -> None:
        """Fit corpus-dependent features when the autoencoder backend is active."""
    ...
    def build_matrix(self, bars: Sequence[BarRecord]) -> np.ndarray:
        """Return a symmetric ``float64[len(bars), len(bars)]`` distance matrix."""
    ...
    def distance(self, left: BarRecord, right: BarRecord) -> float:
        """Return the configured normalized distance between two bars."""
        matrix = self.build_matrix([left, right])
        return float(matrix[0, 1])

    def diagnostics(self, matrix: np.ndarray) -> Dict[str, Any]:
        """Return distribution diagnostics for a pairwise distance matrix."""
    ...
    def _encode(self, tokens: Sequence[int]) -> str:
        """Map signed integer tokens to a string accepted by Levenshtein."""
    ...
    def _distance_pair(
        ...
        """Combine the selected token distance with optional context penalties."""
    ...
    def _multi_channel_distance(self, left: BarRecord, right: BarRecord) -> float:
        """Blend rhythm, contour, and relative-pitch edit distances by weight."""
    ...
    def _rhythm_tokens(self, tokens: Sequence[int]) -> List[int]:
        """Collapse pitches into rest, sustain, and onset rhythm states."""
        result: List[int] = []
        for token in tokens:
            token = int(token)
            if token == -1:
    ...
    def _contour_tokens(self, tokens: Sequence[int]) -> List[int]:
        """Encode local pitch direction while retaining rest and sustain states."""
        result: List[int] = []
    ...
    def _token_edit_distance(self, left_tokens: Sequence[int], right_tokens: Sequence[int]) -> float:
        """Encode integer token rows and compute their configured distance."""
    ...
    def _encoded_edit_distance(self, encoded_left: str, encoded_right: str) -> float:
        """Compute Levenshtein distance with optional length normalization."""
    ...
    def _root_pitch_class_distance(self, left: BarRecord, right: BarRecord) -> float:
        """Return normalized circular distance between optional root pitch classes."""
    ...
    def _bar_root_pitch_class(self, bar: BarRecord) -> Optional[int]:
        """Use the lowest pitched token as a stable bar-root approximation."""
    ...
    def _position_bucket_distance(self, left: BarRecord, right: BarRecord) -> float:
        """Return normalized distance between optional song-position buckets."""
    ...
    def _position_bucket(self, bar: BarRecord) -> Optional[int]:
        """Map a bar index to a bucket over normalized song position."""
