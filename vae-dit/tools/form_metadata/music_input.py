#!/usr/bin/env python3
"""Input parsing and bar preprocessing for the DFA/HMM engine."""
...
class InputParserConfig:
    """Configure score quantization and fixed-grid bar tokenization."""
...
class InputParser:
    ...
    def __init__(self, config: InputParserConfig) -> None:
        """Initialize parser policy, recoverable failures, and feature extraction."""
        self.config = config
    ...
    def from_style_config(cls, config: Dict[str, Any]) -> "InputParser":
        """Build parser settings from the ``grid_tokenizer`` section."""
    ...
    def parse_directory(self, music_dir: str | Path) -> List[SongRecord]:
        """Parse all supported files while recording recoverable file failures."""
    ...
    def parse_file(self, file_path: str | Path, metadata: Dict[str, Any]) -> SongRecord:
        """Parse and tokenize one symbolic score into ordered bar records."""
        ...
        # Ceiling keeps a final partial bar instead of silently dropping notes
        # whose end time is not an exact multiple of the configured bar length.
    ...
    def _discover_files(self, music_dir: Path) -> List[Path]:
        """Return recursively discovered input files in deterministic order."""
        files: List[Path] = []
    ...
    def _load_form_map(self, music_dir: Path) -> Dict[str, Dict[str, Any]]:
        """Load optional metadata keyed by exact source file name."""
    ...
    def _quantize_score(self, score: stream.Score) -> stream.Score:
        """Quantize expressive offsets and durations to stable score fractions."""
    ...
    def _collect_note_events(self, score: stream.Score) -> List[tuple[float, float, int, int]]:
        """Flatten notes and chords to sorted `(start, end, pitch, velocity)` rows."""
    ...
    def _event_pitch(self, element: Any) -> Optional[int]:
        """Select one representative MIDI pitch from a note or chord event."""
    ...
    def _build_bar(
        ...
        """Clip overlapping score events into one bar-local quarter-length window."""
    ...
    def _assign_form_section(self, bar: BarRecord, metadata: Dict[str, Any]) -> None:
        """Attach the first half-open metadata section containing the bar index."""
    ...
    def _preprocess_bar(self, bar: BarRecord) -> None:
        """Populate absolute tokens, relative tokens, and explicit bar features."""
    ...
    def _tokenize_bar(self, bar: BarRecord) -> List[int]:
        """Convert notes to fixed onset/sustain/rest tokens over one bar grid."""
        ...
        # Onsets take precedence over sustains. Polyphony is collapsed using the
        # configured representative-pitch policy so each slot has one token.
    ...
    def _relative_tokens(self, tokens: Sequence[int]) -> List[int]:
        """Express pitched tokens relative to the lowest pitch in the bar."""
    ...
    def _quantize_slot(self, onset_ql: float, slot_len: float) -> int:
        """Map a quarter-length onset to a zero-based fixed-grid slot."""
