...
class BarAutoencoderConfig:
    """Configure the narrow token autoencoder used by edit distance."""
...
class BarTokenAutoencoderFeatureExtractor:
    ...
    def __init__(self, config: BarAutoencoderConfig) -> None:
        """Initialize an unfitted extractor and its corpus statistics."""
        self.config = config
    ...
    def fit(self, bars: Sequence[BarRecord]) -> None:
        """Train on fixed-length bar tokens and cache one latent vector per bar."""
        bars = list(bars)
        ...
        if self.config.normalize_latent:
            # Corpus standardization gives each latent axis comparable influence
            # before it is quantized into edit-distance tokens.
    ...
    def vector_for(self, bar: BarRecord) -> np.ndarray:
        """Return the cached ``float64[latent_dim]`` vector for a fitted bar."""
    ...
    def _validate_bars(self, bars: Sequence[BarRecord]) -> None:
        """Require one fixed token length so the dense autoencoder is well-defined."""
    ...
    def _tokens_for_bar(self, bar: BarRecord) -> List[int]:
        """Read the configured absolute or relative token representation."""
    ...
    def _build_vocab(self, token_rows: Sequence[Sequence[int]]) -> None:
        """Record the observed token domain for diagnostics and compatibility."""
    ...
    def _train_and_encode(self, encoded_rows: np.ndarray) -> tuple[np.ndarray, Dict[str, Any]]:
        """Fit a linear bottleneck autoencoder and return latent rows plus metrics."""
        try:
            import torch
        ...
        class TokenAutoencoder(nn.Module):
            """Map a fixed token vector through a learned linear bottleneck."""
            ...
            def __init__(
                ...
                """Create linear encoder and decoder layers for one bar vector."""
                super().__init__()
            ...
            def forward(self, tokens: "torch.Tensor") -> tuple["torch.Tensor", "torch.Tensor"]:
                """Return reconstructed tokens and their latent representation."""
                latent = self.encoder(tokens)
                reconstruction = self.decoder(latent)
                return reconstruction, latent

            def encode(self, tokens: "torch.Tensor") -> "torch.Tensor":
                """Project normalized token rows to the latent space only."""
    ...
    def _rounded_vector(self, vector: Optional[np.ndarray]) -> Optional[List[float]]:
        """Convert an optional NumPy vector to compact JSON-safe diagnostics."""
        if vector is None:
            return None
