...
class SemanticBarTensorCodec:
    ...
    def __init__(self, config: SemanticBarTensorConfig, projector: FixedChromagramProjector | None = None) -> None:
        """Store the `[tracks, steps, features]` layout and chroma projector."""
        self.config = config
    ...
    def encode(self, bar: BarRecord) -> BarTensorRecord:
        ...
        # Assign voices before writing features so each tensor channel retains a
        # stable melody, harmony, or bass meaning across the dataset.
    ...
    def _bar_base_pitch(self, notes: Sequence[NoteEvent]) -> int | None:
        """Return the lowest physical pitch as the bar-relative anchor."""
        return bass_anchor_pitch(notes)

    def _relative_chromagram(self, notes: Sequence[NoteEvent], base_pitch: int | None) -> np.ndarray:
        """Return duration-and-velocity-weighted chroma relative to the anchor."""
        return relative_chromagram(
            notes,
