...
class FinalV2EvaluationRawCapture:
    ...
    def _parser_integrity(common: Mapping[str, Any], songs: Sequence[SongRecord], failures: Sequence[Mapping[str, Any]], manifest: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
        ...
        initial = [song.metadata.get("initial_time_signature", {}) for song in songs]
        injections = [{"source_file_identity": song.metadata.get("source_file_identity"), **item} for song, item in zip(songs, initial) if item.get("origin") == "smf_default"]
        paired = sum(int(song.metadata.get("paired_source_note_count", 0)) for song in songs)
        dropped = expected["same_tick_zero_duration_pair"]
        return {"schema_version": "parser_integrity_raw_observation.v2", "status": "AVAILABLE", **common, "availability": {"raw_capture": True, "measure_map": True}, "measure_map": {"song_count": len(songs), "measure_count": len(bars), "meter_distribution": dict(Counter(bar.time_signature for bar in bars)), "opus_tune_count": len(opus_sources), "over_capacity_count": 0}, "initial_time_signature_policy": initial[0].get("policy", "error") if initial else "error", "initial_time_signature_origin_counts": dict(Counter(item.get("origin", "unknown") for item in initial)), "initial_time_signature_injections": injections, "partial_span_count": len(partials), "partial_reason_counts": {"time_signature_change": len(partials)}, "partial_spans": partials, "track_retention": {"hard_safety_limit": 48, "policy": "truncate" if any(item.get("policy") == "truncate" for item in retained) else "retain_all", "dropped_part_count": sum(int(item.get("dropped_part_count", 0)) for item in retained), "dropped_note_count": sum(int(item.get("dropped_note_count", 0)) for item in retained), "dropped_note_ratio": float(sum(float(item.get("dropped_note_ratio", 0.0)) for item in retained) / max(1, len(retained)))}, "normalization_policy_version": manifest["normalization_policy_version"], "repair_artifact": artifact, "repair_count": len(repairs), "repair_counts_by_kind": expected, "raw_pairing_repair_counts": expected, "paired_source_note_count": paired, "dropped_source_note_count": dropped, "dropped_source_note_ratio": dropped / (paired + dropped) if paired + dropped else 0.0, "repair_affected_file_count": len({item["source_file_identity"] for item in repairs}), "parser_failures": list(failures), "unavailable_reasons": []}

    @staticmethod
    def _validate_partial_alignment(songs: Sequence[SongRecord], manifest: Mapping[str, Any], output_dir: Path) -> None:
        bars = {(song.song_id, int(bar.canonical_bar_index if bar.canonical_bar_index is not None else bar.bar_index)): (song, bar) for song in songs for bar in song.bars}
        for song in songs:
        ...
        index_path = output_dir / str(manifest.get("index", {}).get("path", "bar_tensor_index.json"))
        arrays_path = output_dir / str(manifest.get("arrays", {}).get("path", ""))
        if not index_path.is_file() or not arrays_path.is_file() or manifest.get("arrays", {}).get("sha256") != _sha256(arrays_path):
            if any(bar.is_partial for _, bar in bars.values()):
        ...
        for position, row in enumerate(rows):
            if row.get("row") != position:
                raise ValueError("canonical partial artifact rows are misaligned")
            item = bars.get((str(row.get("song_id")), int(row.get("canonical_bar_index", -1))))
            if item is None:
    ...
    def _quantization(common: Mapping[str, Any], songs: Sequence[SongRecord], output_dir: Path, policy: SlotGridPolicy) -> dict[str, Any]:
        ...
        for song in songs:
            ...
            repair_counts = audit.get("raw_pairing_repair_counts", {})
            dropped = int(repair_counts.get("same_tick_zero_duration_pair", 0))
            paired = int(audit.get("paired_source_note_count", song.metadata.get("paired_source_note_count", 0)))
            if set(repair_counts) != {"same_tick_zero_duration_pair", "redundant_orphan_note_off"} or dropped != int(audit.get("dropped_source_note_count", -1)) or paired < 0 or not math.isclose(float(audit.get("dropped_source_note_ratio", float("nan"))), dropped / (paired + dropped) if paired + dropped else 0.0, abs_tol=1e-12):
                raise ValueError(f"quantization pairing-loss summary is invalid for {song.song_id}")
        ...
        audits = [song.metadata["quantization_audit"] for song in songs]
        repair_counts = {kind: sum(int(audit["raw_pairing_repair_counts"][kind]) for audit in audits) for kind in ("same_tick_zero_duration_pair", "redundant_orphan_note_off")}
        paired = sum(int(audit["paired_source_note_count"]) for audit in audits)
        dropped = repair_counts["same_tick_zero_duration_pair"]
        return {"schema_version": "quantization_audit_raw_observation.v2", "status": "AVAILABLE", **common, "availability": {"raw_capture": True, "source_boundaries": True, "residual_samples": True}, "audit_unit": "source_note_fragment", "fragment_count": len(flat), "projected_fragment_count": projected, "projected_fragment_rate": projected / len(flat) if flat else 0.0, "raw_pairing_repair_counts": repair_counts, "paired_source_note_count": paired, "dropped_source_note_count": dropped, "dropped_source_note_ratio": dropped / (paired + dropped) if paired + dropped else 0.0, "grid_policy": policy.to_dict(), "by_file_meter": rows, "residual_samples":{"path":path.name,"sha256":_sha256(path),"arrays":arrays}, "unavailable_reasons": []}
    ...
    def _form_action(common: Mapping[str, Any], songs: Sequence[SongRecord]) -> dict[str, Any]:
        ...
        statuses = Counter(str(song.metadata.get("form_mapping_status", "absent")) for song in songs)
        unmapped = [status for status in statuses if status != "mapped"]
        available = not unmapped and bool(bars) and all(any(bar.form for bar in song.bars) for song in songs)
        return {"schema_version": "form_action_alignment_raw_observation.v2", "status": "AVAILABLE" if available else "UNAVAILABLE", **common, "availability": {"raw_capture": True, "form_metadata": available and any(bar.form for bar in bars), "action_labels": any(bar.action for bar in bars)}, "mapping_status_counts": dict(sorted(statuses.items())), "coverage": {"bar_count": len(bars), "form_labeled_bar_count": sum(bool(bar.form) for bar in bars), "action_labeled_bar_count": sum(bool(bar.action) for bar in bars), "joint_labeled_bar_count": sum(bool(bar.form and bar.action) for bar in bars)}, "confusion_table": [{"form": form, "action": action, "count": count} for (form, action), count in sorted(table.items())], "unavailable_reasons": [{"field": "form_metadata", "reason": status} for status in unmapped]}
