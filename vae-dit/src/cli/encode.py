...
class EncodingCLI:
    ...
    def build_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--music-dir", "--dataset-root", dest="music_dir", type=Path, required=True)
        parser.add_argument("--output-dir", type=Path, required=True)
        parser.add_argument("--config", type=Path, default=None)
        parser.add_argument("--schema-version", type=str, default=None)
        return parser
    ...
    def run(self, argv: Optional[Sequence[str]] = None) -> None:
        ...
        configured = config.get("bar_tensor", {}).get("schema_version")
        if args.schema_version is not None and args.schema_version != configured:
            raise SystemExit(f"--schema-version {args.schema_version} does not match configured schema {configured}")
