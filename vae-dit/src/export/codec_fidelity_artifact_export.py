...
def export_codec_fidelity_artifacts(config: CodecFidelityArtifactExportConfig)->None:
    ...
    for split in ("train","validation","excluded_unpaired"):
        for version in ("v1", "v2"):
            status=config.source_dir/f"codec_fidelity__raw_status__{split}.{version}.json"
            ...
            if value.get("status")=="AVAILABLE" and value.get("artifacts", {}).get("observation"):
                obs=json.loads((config.source_dir/value["artifacts"]["observation"]["path"]).read_text(encoding="utf-8"))
                for ref in (obs.get("source_raw", {}), obs.get("encoding_manifest", {}), obs.get("bar_tensor_index", {})):
