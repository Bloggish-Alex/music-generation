#!/usr/bin/env python3
"""CLI for generation with joint latent/register trajectory diffusion."""
...
from pipeline.trajectory_diffusion_pipeline import TrajectoryDiffusionGenerationPipeline


class TrajectoryDiffusionGenerationCLI:
    """Generate with sampled four-bar continuous trajectories and a frozen DVAE."""

    def build_parser(self) -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(description="Generate with joint REMI trajectory diffusion + DVAE decoder.")
    ...
    def run(self, argv: Optional[Sequence[str]] = None) -> None:
        ...
        result = TrajectoryDiffusionGenerationPipeline(config, overrides=overrides).run(
...
def main() -> None:
    TrajectoryDiffusionGenerationCLI().run()
