...
from decoder.form_template import FormTemplate, FormTemplateLibrary
from data.music_input import InputParser, MUSIC_PATTERNS
...
class SSMBuilder:
    """Build bar-level self-similarity matrices from relative bar tokens."""
    ...
    def build_similarity(self, bars: Sequence[Any]) -> np.ndarray:
        ...
        tokens = [self._relative_tokens(bar) for bar in bars]
        n_bars = len(tokens)
        matrix = np.zeros((n_bars, n_bars), dtype=np.float64)
        for row in range(n_bars):
            for col in range(row, n_bars):
                similarity = self._token_similarity(tokens[row], tokens[col])
                matrix[row, col] = similarity
                matrix[col, row] = similarity
        return matrix

    def _relative_tokens(self, bar: Any) -> List[int]:
        if hasattr(bar, "relative_tokens"):
            return [int(token) for token in bar.relative_tokens]
        return [int(token) for token in bar.tokens_for_encoder("relative")]

    def _token_similarity(self, left: Sequence[int], right: Sequence[int]) -> float:
        width = max(len(left), len(right), 1)
        matches = 0.0
        for index in range(width):
            left_token = int(left[index]) if index < len(left) else self.rest_token
            right_token = int(right[index]) if index < len(right) else self.rest_token
            if left_token == right_token:
                matches += 1.0
            elif left_token in {self.rest_token, self.sustain_token} and right_token in {self.rest_token, self.sustain_token}:
                matches += 0.5
        return float(matches / width)
