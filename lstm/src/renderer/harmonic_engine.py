...
from common.config_loader import ConfigLoader, ConfigView, ROOT_DIR
from data.core_data import NoteRecord
from data.bar_density import TokenDensityAnalyzer
from decoder.candidate_selector import (
...
class HarmonicEngine:
    ...
    def _select_candidate(
        ...
        if str(getattr(sampled, "selection_mode", "")) in {"joint_observation_candidate", "lstm_token_logits"}:
            selected = self._entry_from_sampled_bar(sampled, entry.candidates)
            return RareBarSelection(entry=selected, diagnostics={
                "used": True,
                "backend": str(getattr(sampled, "selection_mode", "decoder_selected_candidate")),
