...
class LeftToRightFormHMM:
    ...
    def from_style_config(
        ...
        if backend != "numpy":
            raise ValueError("section_hmm.backend must be 'numpy'.")
        return cls(
            n_states,
            n_observations,
            FormHMMConfig(
    ...
    def from_dict(cls, payload: Dict[str, Any]) -> "LeftToRightFormHMM":
        config_payload = {
            ...
            if key in FormHMMConfig.__dataclass_fields__
        }
        model = cls(
            int(payload["n_states"]),
            int(payload["n_observations"]),
            FormHMMConfig(**config_payload),
...
class FormHMMTrainer:
    ...
    def train(self, songs: Sequence[SongRecord], vocab: ObservationVocab) -> Dict[str, LeftToRightFormHMM]:
        ...
        models: Dict[str, LeftToRightFormHMM] = {}
...
class FormHMMCLI:
    """Standalone CLI for training form HMMs from encoded songs."""
