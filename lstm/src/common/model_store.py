#!/usr/bin/env python3
"""Model bundle persistence for the symbolic music engine."""
...
from common.architecture import EncoderModel, SymbolVocabulary
from data.core_data import BarRecord, ObservationVocab, SongRecord
from decoder.candidate_selector import CandidateSelectorModel
from decoder.form_template import TemplateFormModel
from data.generation_data import CodebookEntry


class ObservationBarPoolBuilder:
    """Build concrete bar pools indexed by contiguous observation ID."""
...
class ModelBundle:
    ...
    def from_training(
        ...
        encoder_model = encoder_model or EncoderModel.from_legacy(
            ...
            vocab,
            metadata={
                "source": "observation_vocab_fallback",
...
def load_form_sequence_model(payload: Dict[str, Any]) -> Any:
    model_type = str(payload.get("model_type", "template_form_model"))
    if model_type == "template_form_model":
        return TemplateFormModel.from_dict(payload)
