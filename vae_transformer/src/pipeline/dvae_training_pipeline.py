...
class DVAETrainingPipeline:
    ...
    def _split_dataset(self, dataset: TensorDataset) -> tuple[TensorDataset, TensorDataset]:
        ...
        size = len(dataset)
        val_size = int(round(size * max(0.0, min(0.9, float(self.training_config.validation_ratio)))))
        if size > 1:
            val_size = max(1, val_size)
        train_size = max(1, size - val_size)
        generator = torch.Generator().manual_seed(int(self.training_config.random_seed))
        train_subset, val_subset = torch.utils.data.random_split(dataset, [train_size, size - train_size], generator=generator)
        train_tensors = dataset.tensors[0][train_subset.indices]
        val_tensors = dataset.tensors[0][val_subset.indices] if val_subset.indices else dataset.tensors[0][:0]
        self.diagnostics.record_stage("dataset_split", {
            "total_size": int(size),
            "train_size": int(len(train_tensors)),
            "validation_size": int(len(val_tensors)),
        })
    ...
    def _run_epoch(
        ...
        for (batch,) in loader:
            ...
            with torch.set_grad_enabled(is_train):
                output = model(batch, add_noise=is_train)
        ...
        if count == 0:
            return {key: 0.0 for key in ["total_loss", "pitch_loss", "state_loss", "velocity_loss", "chord_loss", "kl_loss"]}
